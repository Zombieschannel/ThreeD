...
namespace DDD
{
	...
	public:
		...
		void setHighlights(std::int32_t highlight)
		{
		...
		bool loadFromFile(const std::string& filename)
		{
			...
			if (!t.open(filename))
				return 0;
			return loadFromStream(t);
		}
		bool loadFromMemory(const std::string& file)
		{
			sf::MemoryInputStream t;
			t.open(file.c_str(), file.size());
			return loadFromStream(t);
		}
	};
}